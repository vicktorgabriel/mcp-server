...
async function collectRuntimeStatus() {
  ...
  return {
    ...
    isolation: {
      fileIsolation: {
        active: Boolean(osCaps.hasBwrap),
        mechanism: osCaps.mechanism,
        description: osCaps.hasBwrap ? 'Aislamiento de sistema de archivos activo (namespaces Bubblewrap con bind RO/RW/tmpfs)' : 'Sin aislamiento de sistema de archivos en el SO'
      },
      networkIsolation: {
        active: Boolean(osCaps.hasBwrap),
        unshared: Boolean(osCaps.hasBwrap),
        description: osCaps.hasBwrap ? 'Aislamiento de red activo (--unshare-net, sin acceso a red externa)' : 'Sin aislamiento de red en el SO'
      },
      resourceLimits: {
        active: resourceCaps.active,
        ...
        description: resourceCaps.active
          ? `Límites de recursos reales activos vía ${resourceCaps.activeBackend} (CPU: ${resourceCaps.cpuLimit || 'sin límite'}, RAM: ${resourceCaps.memoryLimit || 'sin límite'}, Procesos: ${resourceCaps.processLimit || 'sin límite'})`
