...
const { detectOsIsolationCapabilities, detectResourceLimitsCapabilities } = require('./sandbox');
...
async function collectRuntimeStatus() {
  ...
  } else if (mode === 'direct' || mode === 'cloudflare') {
  ...
  if (mode === 'cloudflare' && !publicUrl) warnings.push('Cloudflare Tunnel requiere PUBLIC_BASE_URL configurada.');
  if (mode === 'cloudflare' && runtimeRaw && runtimeRaw.tunnelRunning === false && runtimeActive) {
    warnings.push('El proceso cloudflared no figura en ejecución en el supervisor.');
  }
  ...
  if (runtimeActive && runtimeLaunchMode === 'temporary') {
    notes.push('Modo temporal activo: cerrar la terminal o pulsar Ctrl+C detiene MCP y el túnel.');
  }
  if (persistentActive) {
  ...
  const exposureHealthy = mode === 'ngrok'
    ? Boolean(liveTunnel)
  ...
  const osCaps = detectOsIsolationCapabilities(combinedEnv);
  const resourceCaps = detectResourceLimitsCapabilities(combinedEnv);
  ...
  return {
    ...
    isolation: {
      configurado: {
        isolationRequired: Boolean(combinedEnv.MCP_REQUIRE_OS_ISOLATION === '1' || (accessPolicy && accessPolicy.profile === 'trabajo_restringido')),
        limitsRequired: Boolean(resourceCaps.required),
        limitsConfigured: Boolean(resourceCaps.limitsConfigured),
        cpuLimit: resourceCaps.cpuLimit || null,
        memoryLimit: resourceCaps.memoryLimit || null,
        processLimit: resourceCaps.processLimit || null
      },
      disponible: {
        bwrapAvailable: Boolean(osCaps.hasBwrap),
        systemdAvailable: Boolean(resourceCaps.hasSystemd),
        prlimitAvailable: Boolean(resourceCaps.hasPrlimit),
        availableBackends: resourceCaps.availableBackends || []
      },
      aplicado: {
        fileIsolation: null,
        ...
        resourceLimits: null,
        verified: false,
        reason: 'El diagnostico global no verifica un trabajo en ejecucion.',
        isolationMechanism: osCaps.mechanism,
        limitsBackend: resourceCaps.activeBackend || 'none'
      },
      fileIsolation: {
        active: null,
        available: Boolean(osCaps.hasBwrap),
        mechanism: osCaps.mechanism,
        description: osCaps.hasBwrap ? 'Bubblewrap disponible; aplicacion por trabajo no verificada' : 'Sin aislamiento de sistema de archivos en el SO'
      },
      networkIsolation: {
        active: null,
        available: Boolean(osCaps.hasBwrap),
        unshared: null,
        description: osCaps.hasBwrap ? 'Aislamiento de red disponible; aplicacion por trabajo no verificada' : 'Sin aislamiento de red en el SO'
      },
      resourceLimits: {
        active: null,
        available: resourceCaps.active,
        backend: resourceCaps.activeBackend,
        limitsConfigured: resourceCaps.limitsConfigured,
        required: resourceCaps.required,
        cpuLimit: resourceCaps.cpuLimit,
        memoryLimit: resourceCaps.memoryLimit,
        processLimit: resourceCaps.processLimit,
        description: resourceCaps.active
          ? `Limites configurados; aplicacion no verificada via ${resourceCaps.activeBackend} (CPU: ${resourceCaps.cpuLimit || 'sin límite'}, RAM: ${resourceCaps.memoryLimit || 'sin límite'}, Procesos: ${resourceCaps.processLimit || 'sin límite'})`
          : 'Límites de recursos (cgroups v2/prlimit) no configurados o inactivos (distinto e independiente del aislamiento de archivos/red)'
      }
    },
    launch: {
