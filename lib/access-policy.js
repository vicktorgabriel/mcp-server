...
const TOOL_REQUIREMENTS = Object.freeze({
  ...
  job_list_mine: ['command_execution'],
  job_tail_output: ['command_execution'],

  // Code inspection, analysis and safe project modifications.
  code_search_symbols: ['files_read'],
  project_dependency_map: ['files_read'],
  patch_preview: ['files_read'],
  patch_apply: ['files_write'],
  file_restore_safe: ['files_write'],
  project_test_runner: ['command_execution'],

  // Additional diagnostics and security audit.
  service_diagnostics: ['system_read'],
  security_block_history: ['diagnostics']
});

const TOOL_RISK = Object.freeze({
  // Critico: destructive, power, package manager, firewall, mounts, delete, process kill
  file_delete: 'critico',
  package_action: 'critico',
  firewall_action: 'critico',
  mount_action: 'critico',
  power_action: 'critico',
  container_compose: 'critico',
  process_signal: 'critico',
  // Medio: mutating, command execution, desktop control, writing
  write_file: 'medio',
  patch_file: 'medio',
  patch_apply: 'medio',
  file_restore_safe: 'medio',
  file_copy: 'medio',
  file_move: 'medio',
  archive_create: 'medio',
  archive_extract: 'medio',
  run_command: 'medio',
  process_start: 'medio',
  git_command: 'medio',
  tmux_create: 'medio',
  tmux_send: 'medio',
  tmux_interrupt: 'medio',
  tmux_kill: 'medio',
  desktop_open: 'medio',
  window_action: 'medio',
  mouse_move: 'medio',
  mouse_click: 'medio',
  mouse_scroll: 'medio',
  keyboard_hotkey: 'medio',
  keyboard_type: 'medio',
  download_file: 'medio',
  service_action: 'medio',
  job_start: 'medio',
  job_cancel: 'medio',
  project_test_runner: 'medio'
  // Todos los demas son 'bajo' (solo lectura / consulta)
});

const TOOL_CATEGORIES = Object.freeze({
  diagnostics: 'Diagnóstico y Estado',
  files_read: 'Archivos (Lectura)',
  files_write: 'Archivos (Modificación)',
  command_execution: 'Ejecución de Comandos',
  system_read: 'Sistema (Lectura)',
  system_manage: 'Sistema (Gestión)',
  git_read: 'Git (Lectura)',
  git_write: 'Git (Operaciones)',
  tmux_read: 'tmux (Lectura)',
  tmux_write: 'tmux (Control)',
  desktop_view: 'Escritorio (Visualización)',
  desktop_control: 'Escritorio (Control)',
  camera: 'Cámara',
  audio: 'Audio',
  network: 'Red y Conectividad',
  packages: 'Gestor de Paquetes',
  firewall: 'Cortafuegos (Firewall)',
  mounts: 'Montajes de Almacenamiento',
  containers: 'Contenedores Docker',
  power: 'Control de Energía'
});

class BlockHistoryRecorder {
  constructor(maxEvents = 500) {
    this.maxEvents = maxEvents;
    this.events = [];
  }

  record({ clientId, tool, reason, profile, requiredGroups, ip } = {}) {
    const entry = {
      id: `blk_${crypto.randomBytes(8).toString('hex')}`,
      timestamp: new Date().toISOString(),
      clientId: String(clientId || 'anonymous'),
      tool: String(tool || 'unknown'),
      reason: String(reason || 'Bloqueado por política'),
      profile: String(profile || 'unknown'),
      requiredGroups: Array.isArray(requiredGroups) ? requiredGroups : [],
  ...
  getHistory({ clientId, limit = 50, since } = {}) {
    let filtered = clientId ? this.events.filter(e => e.clientId === String(clientId)) : [];
    if (since) {
    ...
    const slice = filtered.slice(-lim).reverse();
    return {
      blocks: slice,
  ...
  query(options = {}) {
    const res = this.getHistory(options);
    return res.blocks;
  }
...
const globalBlockRecorder = new BlockHistoryRecorder();
...
function createAccessPolicy(env = process.env, allToolNames = [], clientPolicyStore = null) {
  ...
  function assertAllowed(toolName, clientContext = {}) {
    ...
    const blockReason = `Bloqueada por perfil de acceso ${effectiveProfile}${needed}`;
    globalBlockRecorder.record({
      clientId,
      tool: toolName,
      reason: blockReason,
      profile: effectiveProfile,
      requiredGroups: requirements
    });
  ...
  return {
    ...
    blockRecorder: globalBlockRecorder,
    profile,
