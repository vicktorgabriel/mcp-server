...
function normalizeState(input) {
  ...
  return {
    ...
    elevationTokens: source.elevationTokens && typeof source.elevationTokens === 'object' ? source.elevationTokens : {},
    mfaExpected: Boolean(source.mfaExpected)
  };
}
  ...
  acquireLock() {
    ...
    while (true) {
      try {
        ...
        let rawOwner = '';
        try {
          rawOwner = fs.readFileSync(this.lockPath, 'utf8').trim();
          const lockAge = Date.now() - fs.statSync(this.lockPath).mtimeMs;
          const ownerPid = Number(rawOwner.split(':', 1)[0]);
          if (lockAge >= STATE_LOCK_TIMEOUT_MS) {
            staleOwner = true;
          } else if (lockAge > 2000 && Number.isInteger(ownerPid) && ownerPid > 0) {
        ...
        if (staleOwner && rawOwner) {
          try {
            if (fs.readFileSync(this.lockPath, 'utf8').trim() === rawOwner) {
...
function authorizationPageCsp(issuer, redirectUri = '', nonce = '') {
  ...
  return `default-src 'none'; script-src ${scriptPolicy}; style-src 'unsafe-inline'; connect-src 'self' ${formOrigin}; form-action ${[...formActionOrigins].join(' ')}; frame-ancestors 'self' https://chatgpt.com https://*.chatgpt.com https://chat.openai.com; base-uri 'none'`;
}
...
class OAuthProvider {
  constructor(options = {}) {
    ...
    const defaultMfaPath = options.storePath
      ? path.join(path.dirname(path.resolve(options.storePath)), 'mfa-state.json')
      : undefined;
    this.mfa = options.mfaManager || new MfaManager({
      filePath: defaultMfaPath,
      oauthStore: this.store
    });
    this.store.cleanup();
  }

  generateElevationToken() {
    const token = `mfa_elev_${randomValue(24)}`;
    const expiresAt = nowSeconds() + 300; // 5 minutes
    this.store.mutate(() => {
      this.store.state.elevationTokens = this.store.state.elevationTokens || {};
      this.store.state.elevationTokens[tokenHash(token)] = { expiresAt };
    });
    return token;
  }

  verifyElevationToken(token) {
    if (!token) return false;
    const key = tokenHash(String(token).trim());
    let valid = false;
    this.store.mutate(() => {
      this.store.state.elevationTokens = this.store.state.elevationTokens || {};
      const record = this.store.state.elevationTokens[key];
      if (record && record.expiresAt > nowSeconds()) {
        valid = true;
        delete this.store.state.elevationTokens[key];
      }
    });
    return valid;
  }
  ...
  revokeClientSessions(clientId) {
    return this.store.mutate(() => {
      let count = 0;
      for (const [key, t] of Object.entries(this.store.state.accessTokens || {})) {
        if (t.clientId === clientId) { delete this.store.state.accessTokens[key]; count++; }
      }
      for (const [key, t] of Object.entries(this.store.state.refreshTokens || {})) {
        if (t.clientId === clientId) { delete this.store.state.refreshTokens[key]; count++; }
      }
      for (const [key, t] of Object.entries(this.store.state.authorizationCodes || {})) {
        if (t.clientId === clientId) { delete this.store.state.authorizationCodes[key]; count++; }
      }
      return count;
    });
  }
  ...
  async handle(req, res, url, baseUrl) {
    ...
    if (pathname === '/oauth/mfa/webauthn/register' && req.method === 'GET') {
      await this.renderWebAuthnRegisterPage(req, res, issuer);
      return true;
    }

    if (pathname === '/oauth/mfa/webauthn/register/options' && req.method === 'POST') {
      await this.handleWebAuthnRegisterOptions(req, res, issuer);
      return true;
    }

    if (pathname === '/oauth/mfa/webauthn/register/verify' && req.method === 'POST') {
      await this.handleWebAuthnRegisterVerify(req, res, issuer);
      return true;
    }

    if (pathname === '/oauth/mfa/webauthn/auth/options' && req.method === 'POST') {
      await this.handleWebAuthnAuthOptions(req, res, issuer);
      return true;
    }

    if (pathname === '/oauth/mfa/webauthn/auth/verify' && req.method === 'POST') {
      await this.handleWebAuthnAuthVerify(req, res, issuer);
      return true;
    }
...
<p>Agregalo desde ChatGPT en modo desarrollador y elegí Mixtas. ChatGPT descubrirá estos endpoints y solicitará OAuth al invocar una herramienta protegida.</p></body></html>`;
  ...
  async handleAuthorizationGet(req, res, url, issuer) {
    ...
    try {
      ...
      sendHtml(res, 200, this.renderAuthorizationPage(created.transactionId, request.client, request.redirectUri, '', nonce), { 'content-security-policy': authorizationPageCsp(issuer, request.redirectUri, nonce) });
    } catch (error) {
  ...
  renderAuthorizationPage(transactionId, client, redirectUri, errorMessage = '', nonce = '') {
    const redirectHost = (() => {
    ...
    let mfaStatus = { enabled: false, webauthnEnrolled: false };
    let mfaActive = false;
    if (this.mfa) {
      mfaActive = Boolean(this.mfa.isMfaEnabled());
      mfaStatus = this.mfa.getStatus();
    }
    const mfaField = mfaActive
      ? '<label for="mfa_code">Código MFA o de recuperación</label><input id="mfa_code" name="mfa_code" autocomplete="one-time-code" placeholder="6 dígitos o XXXX-XXXX" required>'
      : '';
    const webauthnSection = (mfaStatus.webauthnEnrolled)
      ? `<div id="webauthn-box" style="margin-top:14px;border-top:1px solid #e2e8f0;padding-top:14px;text-align:center;">
          <button type="button" id="passkey-btn" class="primary" style="background:#0f766e;margin-top:0;">Identificarse con Passkey / Llave</button>
          <p id="passkey-status" class="muted" style="font-size:0.85rem;margin:6px 0 0 0;"></p>
        </div>`
      : '';
    const webauthnScript = (mfaStatus.webauthnEnrolled)
...
(function() {
  ...
  btn.addEventListener('click', async function() {
    ...
    if (!userField || !passField || !userField.value || !passField.value) {
      if (status) status.innerText = 'Ingresá primero tu usuario y contraseña.';
      return;
    }
    if (status) status.innerText = 'Solicitando desafío a la llave...';
    try {
      const optRes = await fetch('/oauth/mfa/webauthn/auth/options', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          transaction: ${JSON.stringify(transactionId).replace(/</g, '\\u003c')},
      ...
      if (!optRes.ok || !optData.ok) {
        throw new Error(optData.error || 'Credenciales no válidas');
      }
      ...
      if (status) status.innerText = 'Verificando llave...';
      const verifyRes = await fetch('/oauth/mfa/webauthn/auth/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          transaction: ${JSON.stringify(transactionId).replace(/</g, '\\u003c')},
      ...
      if (!verifyRes.ok || !verifyData.ok) {
        throw new Error(verifyData.error || 'Fallo de verificación de la llave');
      }
      if (status) status.innerText = '¡Autenticado! Redirigiendo...';
      window.location.href = verifyData.redirectUrl;
    } catch (err) {
      if (status) status.innerText = 'Error con la llave: ' + err.message;
    }
  });
})();
</script>`
...
${webauthnSection}
<form id="deny-form" method="post" action="/oauth/authorize"><input type="hidden" name="transaction" value="${htmlEscape(transactionId)}"><input type="hidden" name="decision" value="deny"><button class="deny" type="submit">Cancelar</button></form></main>
${webauthnScript}
</body></html>`;
  }
  ...
  async handleAuthorizationPost(req, res, issuer) {
    ...
    try {
      ...
      rejectDuplicateParameters(form, ['transaction', 'decision', 'username', 'password', 'mcp_user', 'mcp_secret', 'mfa_code', 'totp_code', 'recovery_code']);
      ...
      const outcome = this.store.mutate(() => {
        ...
        let mfaActive = false;
        try {
          mfaActive = Boolean(this.mfa && this.mfa.isMfaEnabled());
        } catch (mfaErr) {
          return { ...base, type: 'mfa_corrupted', client, error: mfaErr.message };
        }

        if (mfaActive) {
          const mfaInput = String(form.get('mfa_code') || form.get('totp_code') || form.get('recovery_code') || '').trim();
          if (!mfaInput) {
            return { ...base, type: 'mfa_required', client };
          }
          let mfaOk = false;
          try {
            if (mfaInput.includes('-')) {
              this.mfa.verifyRecoveryCode(mfaInput);
              mfaOk = true;
            } else {
              this.mfa.verifyTotp(mfaInput);
              mfaOk = true;
            }
          } catch (_) {
            mfaOk = false;
          ...
          if (!mfaOk) {
            return { ...base, type: 'mfa_bad_code', client };
          }
        }
      ...
      if (outcome.type === 'bad_credentials') {
        ...
        const nonce = crypto.randomBytes(16).toString('base64');
        sendHtml(res, 401, this.renderAuthorizationPage(transactionId, outcome.client, outcome.transaction.redirectUri, 'Usuario o contraseña incorrectos.', nonce), { 'content-security-policy': authorizationPageCsp(issuer, outcome.transaction.redirectUri, nonce) });
        return;
      }
      if (outcome.type === 'mfa_required' || outcome.type === 'mfa_bad_code') {
        const errorMsg = outcome.type === 'mfa_bad_code' ? 'Código MFA o de recuperación incorrecto.' : 'Se requiere código MFA.';
        if (outcome.type === 'mfa_bad_code') this.loginLimiter.recordFailure(ip);
        const nonce = crypto.randomBytes(16).toString('base64');
        sendHtml(res, 401, this.renderAuthorizationPage(transactionId, outcome.client, outcome.transaction.redirectUri, errorMsg, nonce), { 'content-security-policy': authorizationPageCsp(issuer, outcome.transaction.redirectUri, nonce) });
        return;
      }
      if (outcome.type === 'mfa_corrupted') {
        this.loginLimiter.recordFailure(ip);
        humanEvent('SEGURIDAD', `Intento OAuth bloqueado desde ${ip}: estado MFA corrupto o ausente.`);
        const nonce = crypto.randomBytes(16).toString('base64');
        sendHtml(res, 401, this.renderAuthorizationPage(transactionId, outcome.client, outcome.transaction.redirectUri, 'Estado de MFA ausente o corrupto. Autenticación bloqueada por seguridad.', nonce), { 'content-security-policy': authorizationPageCsp(issuer, outcome.transaction.redirectUri, nonce) });
        return;
      }
  ...
  async readJsonOrForm(req, limit = 64 * 1024) {
    const raw = await readBody(req, limit);
    ...
    if (contentType.includes('application/json')) {
      try {
        ...
        throw new OAuthError('invalid_request', 'JSON inválido', 400);
      }
    }
  ...
  async handleWebAuthnRegisterOptions(req, res, issuer) {
    try {
      const ip = remoteAddress(req);
      const limit = this.loginLimiter.status(ip);
      if (!limit.allowed) {
        return sendJson(res, 429, { ok: false, error: `Demasiados intentos. Esperá ${limit.retryAfter}s.` });
      }

      const requestOrigin = String(req.headers.origin || '').trim();
      if (requestOrigin && normalizeBaseUrl(requestOrigin) !== normalizeBaseUrl(issuer)) {
        return sendJson(res, 403, { ok: false, error: 'Origin no coincide con el servidor.' });
      }

      const data = await this.readJsonOrForm(req);
      const username = String(data.username || '');
      const password = String(data.password || '');
      const admin = this.store.state.admin;
      if (!admin || !timingSafeTextEqual(username, admin.username) || !verifyPassword(password, admin.passwordHash)) {
        this.loginLimiter.recordFailure(ip);
        return sendJson(res, 401, { ok: false, error: 'Credenciales inválidas para enrolamiento WebAuthn.' });
      }

      // If MFA is already active, require elevation with existing factor or local elevation token
      let isMfaActive = false;
      try {
        isMfaActive = Boolean(this.mfa && this.mfa.isMfaEnabled());
      } catch (err) {
        return sendJson(res, 500, { ok: false, error: err.message });
      }

      if (isMfaActive) {
        const mfaCode = String(data.mfa_code || data.totp_code || data.recovery_code || '').trim();
        const elevationToken = String(data.elevation_token || data.elevationToken || '').trim();
        let elevated = false;
        if (mfaCode) {
          if (mfaCode.includes('-')) {
            try {
              this.mfa.verifyRecoveryCode(mfaCode);
              elevated = true;
            } catch (_) {}
          } else {
            try {
              this.mfa.verifyTotp(mfaCode);
              elevated = true;
            } catch (_) {}
          }
        } else if (elevationToken) {
          elevated = this.verifyElevationToken(elevationToken);
        }
        if (!elevated) {
          this.loginLimiter.recordFailure(ip);
          return sendJson(res, 403, {
            ok: false,
            error: 'MFA ya está activo. Se requiere elevación con código MFA/recuperación existente o aprobación local del operador para inscribir una nueva llave.'
          });
        }
      }

      const rpId = new URL(issuer).hostname;
      const options = await this.mfa.generateRegistrationOptions({
        userName: admin.username,
        rpId,
        rpName: 'MCP Server',
        elevationVerified: elevated
      });
      return sendJson(res, 200, { ok: true, options, challengeId: options.challengeId });
    } catch (err) {
      return sendJson(res, 400, { ok: false, error: err.message });
    }
  }

  async handleWebAuthnRegisterVerify(req, res, issuer) {
    try {
      const ip = remoteAddress(req);
      const limit = this.loginLimiter.status(ip);
      if (!limit.allowed) {
      ...
      if (!admin || !timingSafeTextEqual(username, admin.username) || !verifyPassword(password, admin.passwordHash)) {
        this.loginLimiter.recordFailure(ip);
        return sendJson(res, 401, { ok: false, error: 'Credenciales inválidas para verificación WebAuthn.' });
      }

      // If MFA is currently active, require valid elevation
      let isMfaActive = false;
      try {
        isMfaActive = Boolean(this.mfa && this.mfa.isMfaEnabled());
      } catch (err) {
      ...
      if (isMfaActive) {
        ...
        if (mfaCode) {
          if (mfaCode.includes('-')) {
            try { this.mfa.verifyRecoveryCode(mfaCode); elevated = true; } catch (_) {}
          } else {
            try { this.mfa.verifyTotp(mfaCode); elevated = true; } catch (_) {}
          }
        } else if (elevationToken) {
          elevated = this.verifyElevationToken(elevationToken);
        }
        if (!elevated) {
          const mfaState = this.mfa._readState ? this.mfa._readState() : null;
          const ch = mfaState && mfaState.activeChallenges ? mfaState.activeChallenges[challengeId] : null;
          if (ch && ch.elevationVerified && ch.mfaEnabledAtIssue) {
            elevated = true;
          }
        }
        if (!elevated) {
          ...
          return sendJson(res, 403, {
            ok: false,
            error: 'MFA ya está activo. Se requiere elevación para verificar e inscribir una nueva llave.'
          });
        }
      }

      const rpId = new URL(issuer).hostname;
      const origin = normalizeBaseUrl(issuer);
      const result = await this.mfa.verifyRegistrationResponse({
        challengeId,
        response,
      ...
      this.store.mutate(() => {
        this.store.state.mfaExpected = true;
      });

      return sendJson(res, 200, { ok: true, recoveryCodes: result.recoveryCodes });
    } catch (err) {
      return sendJson(res, 400, { ok: false, error: err.message });
    }
  }

  async handleWebAuthnAuthOptions(req, res, issuer) {
    try {
      const ip = remoteAddress(req);
      const limit = this.loginLimiter.status(ip);
      if (!limit.allowed) {
      ...
      const transaction = this.store.state.authorizationTransactions[key];
      if (!transaction || transaction.expiresAt <= nowSeconds()) {
        return sendJson(res, 400, { ok: false, error: 'Transacción expirada o no encontrada.' });
      }
      const admin = this.store.state.admin;
      if (!admin || !timingSafeTextEqual(username, admin.username) || !verifyPassword(password, admin.passwordHash)) {
        this.loginLimiter.recordFailure(ip);
      ...
      const rpId = new URL(issuer).hostname;
      const { options, challengeId } = await this.mfa.generateAuthenticationOptions({ rpId });
      this.store.mutate(() => {
        if (this.store.state.authorizationTransactions[key]) {
          this.store.state.authorizationTransactions[key].webauthnChallengeId = challengeId;
        }
      });
      return sendJson(res, 200, { ok: true, options, challengeId });
    } catch (err) {
      return sendJson(res, 400, { ok: false, error: err.message });
    }
  }

  async handleWebAuthnAuthVerify(req, res, issuer) {
    try {
      const ip = remoteAddress(req);
      const limit = this.loginLimiter.status(ip);
      if (!limit.allowed) {
      ...
      if (!transaction.webauthnChallengeId || transaction.webauthnChallengeId !== challengeId) {
        return sendJson(res, 400, { ok: false, error: 'Desafío WebAuthn no coincide con la transacción o no fue asignado.' });
      }
      const rpId = new URL(issuer).hostname;
      const origin = normalizeBaseUrl(issuer);
      await this.mfa.verifyAuthenticationResponse({
        challengeId,
        response,
        expectedOrigin: origin,
        expectedRPID: rpId
      ...
      // Issue authorization code with atomic revalidation
      const admin = this.store.state.admin;
      let rawCode = null;
      this.store.mutate(() => {
        const freshTx = this.store.state.authorizationTransactions[key];
        if (!freshTx || freshTx.expiresAt <= nowSeconds() || freshTx.webauthnChallengeId !== challengeId) {
          throw new Error('Transacción ya consumida o modificada concurrentemente.');
        }
        rawCode = `mcp_ac_${randomValue(32)}`;
        this.store.state.authorizationCodes[tokenHash(rawCode)] = {
          clientId: freshTx.clientId,
          redirectUri: freshTx.redirectUri,
          codeChallenge: freshTx.codeChallenge,
          scope: freshTx.scope,
          resource: freshTx.resource,
          subject: admin ? admin.username : 'admin',
          expiresAt: nowSeconds() + this.codeTtl
        };
        delete this.store.state.authorizationTransactions[key];
      });

      const redirectUrl = appendRedirectParams(transaction.redirectUri, {
        code: rawCode,
  ...
  async renderWebAuthnRegisterPage(req, res, issuer) {
    const nonce = crypto.randomBytes(16).toString('base64');
    let mfaActive = false;
    try {
      mfaActive = Boolean(this.mfa && this.mfa.isMfaEnabled());
    ...
    const elevationField = mfaActive
      ? '<label for="mfa_code">Código MFA actual o token de elevación local</label><input id="mfa_code" name="mfa_code" placeholder="Código TOTP, recovery code o token local" required>'
      : '';

    const html = `<!doctype html>
<html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Registrar Llave WebAuthn</title>
<style>body{font-family:system-ui,sans-serif;background:#f4f7fb;margin:0;color:#15202b}.card{max-width:520px;margin:7vh auto;background:white;border-radius:16px;padding:28px;box-shadow:0 12px 40px #10243a22}h1{font-size:1.45rem;margin-top:0}.muted{color:#5d6975;font-size:.94rem}.notice{background:#eef6ff;border:1px solid #bddbff;padding:12px;border-radius:10px;margin:16px 0}.error{background:#fff1f1;border:1px solid #e5a8a8;color:#8b1c1c;padding:10px;border-radius:8px}.success{background:#edfcf2;border:1px solid #a3e635;color:#166534;padding:12px;border-radius:8px}label{display:block;margin-top:14px;font-weight:600}input{box-sizing:border-box;width:100%;padding:11px;margin-top:5px;border:1px solid #bac5d0;border-radius:8px;font-size:1rem}.primary{box-sizing:border-box;width:100%;border:0;border-radius:8px;padding:11px 15px;font-size:1rem;cursor:pointer;background:#0f766e;color:white;margin-top:22px}</style></head>
<body><main class="card"><h1>Registrar Llave WebAuthn / Passkey</h1>
<p class="muted">Configurá una llave física (FIDO2/YubiKey) o datos biométricos locales para autenticar el acceso OAuth.</p>
<div id="status-box"></div>
<form id="reg-form">
<label for="username">Usuario Administrador</label><input id="username" name="username" required>
<label for="password">Contraseña</label><input id="password" type="password" name="password" required>
${elevationField}
<button class="primary" type="submit" id="submit-btn">Comenzar Registro de Llave</button>
</form>
</main>
<script nonce="${nonce}">
(function() {
  ...
  form.addEventListener('submit', async function(e) {
    e.preventDefault();
    btn.disabled = true;
    statusBox.innerHTML = '<p class="notice">Solicitando opciones de registro...</p>';
    try {
      const user = document.getElementById('username').value;
      const pass = document.getElementById('password').value;
      const mfaInput = document.getElementById('mfa_code');
      const payload = { username: user, password: pass };
      if (mfaInput && mfaInput.value) payload.mfa_code = mfaInput.value;

      const optRes = await fetch('/oauth/mfa/webauthn/register/options', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const optData = await optRes.json();
      if (!optRes.ok || !optData.ok) {
        throw new Error(optData.error || 'Fallo al obtener opciones');
      }

      statusBox.innerHTML = '<p class="notice">Tocá tu llave de seguridad o usá biometría en el navegador...</p>';
      const opts = optData.options;
      opts.challenge = Uint8Array.from(atob(opts.challenge.replace(/-/g, '+').replace(/_/g, '/')), c => c.charCodeAt(0));
      ...
      if (mfaInput && mfaInput.value) verifyPayload.mfa_code = mfaInput.value;

      statusBox.innerHTML = '<p class="notice">Verificando registro con el servidor...</p>';
      const verifyRes = await fetch('/oauth/mfa/webauthn/register/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const verifyData = await verifyRes.json();
      if (!verifyRes.ok || !verifyData.ok) {
        throw new Error(verifyData.error || 'Fallo de verificación de registro');
      }

      let recoveryHtml = '';
      if (verifyData.recoveryCodes && verifyData.recoveryCodes.length) {
        recoveryHtml = '<div style="margin-top:10px;text-align:left;"><strong>Códigos de recuperación de un solo uso (guardalos en un lugar seguro):</strong><pre style="background:#f1f5f9;padding:10px;border-radius:6px;font-size:0.9rem;">' + verifyData.recoveryCodes.join('\\n') + '</pre></div>';
      }
      statusBox.innerHTML = '<div class="success"><strong>¡Llave WebAuthn registrada exitosamente!</strong>' + recoveryHtml + '</div>';
    ...
    sendHtml(res, 200, html, { 'content-security-policy': authorizationPageCsp(issuer, '', nonce) });
  }
}
