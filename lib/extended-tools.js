...
// Internal extended tool implementations; mcp-server.js is the public entrypoint.

const crypto = require('crypto');
const { createProjectTools } = require('./tools/project-tools');
const fs = require('fs');
const net = require('net');
const path = require('path');
const { spawn } = require('child_process');
const { validateEgressUrl, validateZipSlip, wrapCommandInSandbox } = require('./sandbox');
...
function requireConfirmation(actual, expected, action, args = {}) {
  if (!CRITICAL_CONFIRMATIONS_REQUIRED) return;
  if (args && args._approvedByApprovalIdVerified === true) {
    return;
  }
...
def target_for(name):
    ...
    sensitive_parts = {'.env', '.runtime', '.git', '.private', '.ssh', '.aws', '.gnupg', 'approvals.json', 'client-policies.json', 'oauth-state.json', 'mfa-state.json', 'mcp.sock', 'mcp.token', '.mfa-configured', '.client-policies-configured'}
    for part in pure.parts:
        low = part.lower()
        if low in sensitive_parts or low.startswith('.env') or low.endswith('.key') or low.endswith('.pem'):
            raise RuntimeError(f'archive entry targets protected/sensitive path: {name}')
...
async function safeFetchWithEgressValidation(initialUrl, fetchOptions = {}, maxRedirects = 5) {
  let currentUrl = String(initialUrl);
  let redirectCount = 0;

  while (true) {
    await validateEgressUrl(currentUrl);
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (redirectCount > maxRedirects) {
        throw new Error(`Demasiadas redirecciones HTTP (límite: ${maxRedirects})`);
      }
...
function createExtendedTools(options = {}) {
  const {
    resolvePath,
    buildToolMetadata,
    textResult,
    ...
    accessPolicy = null,
    jobManager: passedJobManager = null,
    approvals: passedApprovals = null
  } = options;
  const jobManager = passedJobManager || new JobManager();
  const approvals = passedApprovals || new ApprovalsManager();
  const projectTools = createProjectTools({ resolvePath, allowedRoots, accessPolicy, jobManager, execCommand });
  ...
  const tools = [
    ...
    // Nuevas capacidades v4.6
    makeTool(buildToolMetadata, 'code_search_symbols', 'Buscar simbolos', 'Busca definiciones de símbolos (funciones, clases, métodos, variables, tipos, exportaciones) en el proyecto sin ejecutar código, con busqueda literal y paginacion; analisis heuristico, no referencias semanticas.', {
      query: { type: 'string', description: 'Nombre o patrón de símbolo a buscar.' },
      path: { type: 'string', description: 'Ruta relativa donde buscar. Predeterminado ".".' },
      symbolType: { type: 'string', enum: ['any', 'function', 'class', 'variable', 'type', 'export'], description: 'Filtro por tipo de símbolo. Predeterminado "any".' },
      fileExtensions: { type: 'array', items: { type: 'string' }, description: 'Extensiones de archivo a incluir (ej: ["js", "ts", "py"]).' },
      isRegex: { type: 'boolean', description: 'Debe ser false; las expresiones regulares arbitrarias no estan habilitadas.' },
      limit: { type: 'number', description: 'Límite de resultados (predeterminado 20, máximo 100).' },
      offset: { type: 'number', description: 'Desplazamiento para paginación.' }
    }, ['query'], ro),

    makeTool(buildToolMetadata, 'project_dependency_map', 'Inspeccionar dependencias', 'Analiza el mapa de dependencias del proyecto leyendo directamente manifiestos (package.json, requirements.txt, pyproject.toml, go.mod, Cargo.toml) sin ejecutar código.', {
      path: { type: 'string', description: 'Directorio del proyecto. Predeterminado ".".' },
      includeDev: { type: 'boolean', description: 'Incluir dependencias de desarrollo. Predeterminado true.' },
    }, [], ro),

    makeTool(buildToolMetadata, 'patch_preview', 'Previsualizar cambios', 'Previsualiza las modificaciones y genera un diff unificado antes de aplicar parches en el archivo, calculando hashes sha256.', {
    ...
    makeTool(buildToolMetadata, 'patch_apply', 'Aplicar cambios verificados', 'Aplica parches de forma condicionada al hash SHA-256 esperado del archivo, creando respaldo automático para permitir restauración segura.', {
      path: { type: 'string', description: 'Ruta del archivo a modificar.' },
      patches: {
        type: 'array',
        items: {
      ...
      expectedHash: { type: 'string', description: 'Hash SHA-256 esperado del archivo antes de la edición. Si difiere, se rechaza por conflicto concurrente.' },
    }, ['path', 'patches', 'expectedHash'], rw),

    makeTool(buildToolMetadata, 'file_restore_safe', 'Restaurar cambios propios', 'Restaura exclusivamente copias de respaldo propias del servidor MCP, verificando que no se destruyan modificaciones posteriores del usuario.', {
      path: { type: 'string', description: 'Ruta del archivo a restaurar.' },
      backupId: { type: 'string', description: 'Identificador obligatorio de una copia propia de este cliente y archivo.' },
    }, ['path', 'backupId'], rw),

    makeTool(buildToolMetadata, 'project_test_runner', 'Ejecutar pruebas y chequeos', 'Ejecuta suites de prueba, linting o comprobaciones de tipos mediante ejecutables autorizados sin concatenación shell.', {
      script: { type: 'string', description: 'Nombre de script package.json para custom_script.' },
      idempotencyKey: { type: 'string', description: 'Clave propia de reintento; contenido distinto se rechaza.' },
      runner: {
        type: 'string',
        enum: ['npm_test', 'npm_check', 'npm_lint', 'eslint', 'pytest', 'tsc', 'cargo_test', 'go_test', 'custom_script'],
        description: 'Herramienta autorizada a ejecutar.'
      },
      args: { type: 'array', items: { type: 'string' }, description: 'Argumentos adicionales estructurados.' },
      cwd: { type: 'string', description: 'Directorio de trabajo dentro de las rutas permitidas.' },
      timeoutMs: { type: 'number', description: 'Límite de tiempo en milisegundos (predeterminado 60000, máximo 300000).' }
    }, ['runner'], rw),

    makeTool(buildToolMetadata, 'service_diagnostics', 'Diagnosticar servicios permitidos', 'Diagnostica puertos locales, servicios systemd permitidos y conectividad del servidor sin escaneos externos.', {
      checkPorts: { type: 'array', items: { type: 'number' }, description: 'Lista de puertos locales (1-65535) a comprobar en 127.0.0.1.' },
      checkServices: { type: 'array', items: { type: 'string' }, description: 'Lista de nombres de unidades systemd a inspeccionar.' },
      timeoutMs: { type: 'number', description: 'Tiempo de espera en ms (predeterminado 3000).' }
    }, [], ro),

    makeTool(buildToolMetadata, 'security_block_history', 'Consultar mis bloqueos', 'Muestra el historial reciente de herramientas y acciones bloqueadas por perfil de acceso, denylist o falta de aprobación.', {
      limit: { type: 'number', description: 'Cantidad máxima de eventos a devolver (predeterminado 50, máximo 200).' },
      since: { type: 'string', description: 'Filtrar eventos posteriores a esta marca de tiempo ISO.' },
      clientId: { type: 'string', description: 'Filtrar por identificador de cliente (opcional).' }
    }, [], ro),

    makeTool(buildToolMetadata, 'job_list_mine', 'Listar mis trabajos', 'Lista los trabajos en segundo plano pertenecientes al cliente autenticado, con filtro de estado y paginación.', {
      status: { type: 'string', enum: ['all', 'running', 'completed', 'failed', 'canceled', 'timed_out'], description: 'Filtro por estado. Predeterminado "all".' },
      limit: { type: 'number', description: 'Límite de resultados (predeterminado 20, máximo 100).' },
      offset: { type: 'number', description: 'Desplazamiento para paginación.' }
    }, [], ro),

    makeTool(buildToolMetadata, 'job_tail_output', 'Seguir salida de mi trabajo', 'Consulta de forma incremental la salida de un trabajo propio, con soporte para cursor fromOffset y límite de líneas.', {
      jobId: { type: 'string', description: 'Identificador del trabajo.' },
      fromOffset: { type: 'number', description: 'Desplazamiento en bytes desde el inicio (predeterminado 0).' },
      limit: { type: 'number', description: 'Cantidad máxima de bytes a leer (predeterminado 65536).' },
      lineLimit: { type: 'number', description: 'Límite opcional de líneas a devolver.' }
    }, ['jobId'], ro)
  ];

  async function callTool(name, args = {}, context = {}) {
    const projectResult = await projectTools.call(name, args, context);
    if (projectResult !== null) return textResult(projectResult);
    switch (name) {
      case 'directory_tree':
      ...
      case 'file_delete': {
        requireConfirmation(args.confirm, 'DELETE', 'El borrado', args);
      ...
      case 'http_request': {
        const validated = await validateEgressUrl(String(args.url));
        ...
        try {
          ...
          const response = await safeFetchWithEgressValidation(url.href, { method, headers, signal: controller.signal });
          ...
          return textResult({ url: response.url || url.href, status: response.status, ok: response.ok, headers: outputHeaders, bytes, truncated, body });
        } finally {
          clearTimeout(timer);
        }
      }
      ...
      case 'download_file': {
        ...
        try {
          const response = await safeFetchWithEgressValidation(url.href, { signal: controller.signal });
          ...
          fs.renameSync(temporary, destination.fullPath);
          return textResult({ ok: true, url: response.url || url.href, destination: destination.displayPath, bytes, sha256: hash.digest('hex') });
        } catch (error) {
      ...
      case 'package_action': {
        requireConfirmation(args.confirm, 'APPLY PACKAGES', 'La modificación de paquetes', args);
        const manager = await detectPackageManager();
      ...
      case 'firewall_action': {
        requireConfirmation(args.confirm, 'APPLY FIREWALL', 'La modificación del firewall', args);
        const action = String(args.action);
      ...
      case 'mount_action': {
        requireConfirmation(args.confirm, 'APPLY MOUNT', 'La modificación de montajes', args);
        const action = String(args.action);
      ...
      case 'container_compose': {
        ...
        if (mutating) requireConfirmation(args.confirm, 'APPLY CONTAINERS', 'La modificación de contenedores', args);
      ...
      case 'power_action': {
        ...
        requireConfirmation(args.confirm, expected, 'La acción de energía', args);
