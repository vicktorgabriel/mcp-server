...
function redactText(value) {
  return String(value === undefined || value === null ? '' : value)
    ...
    .replace(/(cookie\s*:\s*)\S+/gi, '$1[OCULTO]')
    .replace(/\b(?:mcp_(?:at|rt|ac|cs)_|tx_)[A-Za-z0-9_-]+\b/g, '[OCULTO]')
    .replace(/(https?:\/\/)[^/\s@]+@/gi, '$1[OCULTO]@')
    .replace(/((?:^|\s)--?(?:authtoken|access[_-]?token|refresh[_-]?token|client[_-]?(?:secret|assertion)|code[_-]?(?:verifier|challenge)|password(?:[_-]?hash)?|passwd|secret|state|token|api[_-]?key|authorization|auth|cookie)\s+)\S+/gi, '$1[OCULTO]')
    .replace(/(^|\s)-p[^\s;]+/gi, '$1-p[OCULTO]')
    .replace(/((?:authtoken|access[_-]?token|refresh[_-]?token|client[_-]?(?:secret|assertion)|code(?:[_-]?(?:verifier|challenge))?|password(?:[_-]?hash)?|passwd|secret|state|token|api[_-]?key|authorization|auth|cookie)["']?\s*[:=]\s*["']?)(?!presente\b|ausente\b|correct[oa]\b|incorrect[oa]\b|válid[oa]\b|inválid[oa]\b)[^\s,"'}&;]+/gi, '$1[OCULTO]')
    .replace(/([?&](?:code|state|token|access_token|refresh_token|client_secret|client_assertion|code_verifier|code_challenge)=)[^&\s]+/gi, '$1[OCULTO]');
}
...
function localIsoTimestamp(date = new Date()) {
  const pad = (value, width = 2) => String(Math.trunc(Math.abs(value))).padStart(width, '0');
  ...
  if (/^https?:\/\//i.test(raw)) {
    try {
      ...
      for (const key of [...parsed.searchParams.keys()]) {
        if (/token|code|state|verifier|challenge|assertion|secret|password|passwd|auth|cookie|api[_-]?key/i.test(key)) parsed.searchParams.set(key, '[OCULTO]');
      }
