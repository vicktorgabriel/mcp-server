...
function extractAffectedPaths(args) {
  if (!args || typeof args !== 'object' || Array.isArray(args)) return [];
  const paths = [];
  const keys = ['path', 'source', 'destination', 'target', 'archive', 'cwd', 'repo', 'project', 'file'];
  for (const k of keys) {
...
function describeConsequences(tool, args) {
  const safe = cleanArgsForHash(args || {});
  switch (tool) {
    case 'file_delete':
      return `Eliminación permanente e irreversible de '${safe.path || 'la ruta indicada'}'`;
    case 'power_action':
      return `Reinicio o apagado del equipo (${safe.action || 'acción'}) tras ${safe.delaySeconds || 10} segundos`;
    case 'package_action':
      return `Modificación de paquetes del sistema (${safe.action || 'acción'}): ${Array.isArray(safe.packages) ? safe.packages.join(', ') : 'paquetes'}`;
    case 'firewall_action':
      return `Modificación de reglas de cortafuegos: ${safe.action || 'acción'} regla '${safe.rule || ''}'`;
    case 'mount_action':
      return `${safe.action === 'mount' ? 'Montaje' : 'Desmontaje'} de '${safe.source || ''}' en '${safe.target || ''}'`;
    case 'container_compose':
      return `Modificación de contenedores Docker: ${safe.action || 'acción'} en '${safe.project || '.'}'`;
    case 'process_signal':
      return `Envío de señal ${safe.signal || 'SIGTERM'} al proceso PID ${safe.pid}`;
    case 'service_action':
      return `Modificación de servicio systemd (${safe.action}) en unidad '${safe.service}'`;
    case 'patch_apply':
      return `Modificación de contenido en '${safe.path}' condicionada a verificación de integridad`;
    case 'file_restore_safe':
      return `Restauración de versión previa de '${safe.path}' con verificación de conflictos`;
    default:
      return `Ejecución de acción '${tool}' sujeta a confirmación explícita`;
  }
}
...
class ApprovalsManager {
  ...
  createPendingApproval({ clientId, tool, args, scope = 'tool_execution', policyVersion = '1', ttlMs = DEFAULT_TTL_MS }) {
    ...
    const affectedPaths = extractAffectedPaths(cleanArgs);
    const consequences = describeConsequences(tool, cleanArgs);
