...
/*
 * Internal full-control tools for the local MCP server.
 *
...
const { wrapCommandInSandbox } = require('./sandbox');
...
function createFullControl(options = {}) {
  const { resolvePath, buildToolMetadata, textResult, allowedRoots = [], accessPolicy = null } = options;
  const ro = { readOnlyHint: true, openWorldHint: true };
  const rw = { destructiveHint: true, idempotentHint: false, openWorldHint: true };

  function runScopedCommand(cmd, cmdArgs = [], execOptions = {}) {
    let finalCmd = cmd;
    let finalArgs = cmdArgs.map(String);
    if (cmd === 'git') {
      const safeGitFlags = [
      ...
      finalArgs = [...safeGitFlags, ...finalArgs];
    }
    const isRestricted = Boolean(
      execOptions.isRestrictedExecution ||
      (accessPolicy && accessPolicy.profile === 'trabajo_restringido')
    );
    if (isRestricted) {
      const cwd = execOptions.cwd || (execOptions.repo ? path.resolve(execOptions.repo) : (allowedRoots[0] || process.cwd()));
      const wrapped = wrapCommandInSandbox(finalCmd, finalArgs, { allowedRoots, cwd }, process.env);
      finalCmd = wrapped.command;
      finalArgs = wrapped.args;
    }
    return execCommand(finalCmd, finalArgs, execOptions);
  }

  async function requireScopedSuccess(cmd, cmdArgs = [], execOptions = {}) {
    const result = await runScopedCommand(cmd, cmdArgs, execOptions);
    if (result.exit_code !== 0 || result.timed_out) {
      const detail = (result.stderr || result.stdout || '').trim();
      throw new Error(`${cmd} failed${result.timed_out ? ' (timeout)' : ''}${detail ? `: ${detail}` : ''}`);
    }
    return result;
  }
  ...
  async function callTool(name, args = {}, context = {}) {
    const isRestricted = Boolean(
      (context && context.isRestrictedExecution) ||
      (accessPolicy && accessPolicy.profile === 'trabajo_restringido')
    );
    if (isRestricted && name.startsWith('tmux_')) {
    ...
    const scopedOpts = (extra = {}) => ({
      isRestrictedExecution: isRestricted,
      ...extra
    });
    ...
    switch (name) {
      ...
      case 'process_start': {
        ...
        let spawnCmd = String(args.command);
        let spawnArgs = Array.isArray(args.args) ? args.args.map(String) : [];
        if (accessPolicy && accessPolicy.profile === 'trabajo_restringido') {
          const wrapped = wrapCommandInSandbox(spawnCmd, spawnArgs, { allowedRoots, cwd }, process.env);
      ...
      case 'git_status': {
        ...
        return textResult(await requireScopedSuccess('git', ['-C', repo, 'status', '--short', '--branch'], scopedOpts({ repo })));
      }
      case 'git_diff': {
        const repo = resolvedDirectory(args.repo);
        ...
        return textResult(await requireScopedSuccess('git', cmdArgs, scopedOpts({ repo, outputLimit: 16 * 1024 * 1024 })));
      }
      case 'git_log': {
        const repo = resolvedDirectory(args.repo);
        const limit = clampInt(args.limit, 20, 1, 200);
        return textResult(await requireScopedSuccess('git', ['-C', repo, 'log', '--oneline', '--decorate', `-${limit}`], scopedOpts({ repo })));
      }
      case 'git_branches': {
        const repo = resolvedDirectory(args.repo);
        return textResult(await requireScopedSuccess('git', ['-C', repo, 'branch', '--all', '-vv'], scopedOpts({ repo })));
      }
      case 'git_worktrees': {
        const repo = resolvedDirectory(args.repo);
        return textResult(await requireScopedSuccess('git', ['-C', repo, 'worktree', 'list', '--porcelain'], scopedOpts({ repo })));
      }
      case 'git_command': {
        const repo = resolvedDirectory(args.repo);
        if (!Array.isArray(args.args)) throw new Error('args must be an array');
        return textResult(await runScopedCommand('git', ['-C', repo, ...args.args.map(String)], scopedOpts({ repo, timeoutMs: clampInt(args.timeoutMs, DEFAULT_TIMEOUT_MS, 1000, 600000), outputLimit: 16 * 1024 * 1024 })));
      }
      case 'tmux_list': {
